export function recommendWorkerCount({
  ...
  const memoryCap = Number.isFinite(deviceMemory) && deviceMemory > 0
    ? Math.max(1, Math.floor((deviceMemory * 0.65) / perWorkerGb))
    : 8;

  return Math.max(1, Math.min(safeMaxWorkers, segmentCap, cpuCap, memoryCap, 8));
}

export function splitQueueIntoBatches(items, workerCount) {
