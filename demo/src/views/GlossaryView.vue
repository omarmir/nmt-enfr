  ...
  <main class="flex flex-col gap-6">
    ...
    <section class="rounded-md border border-gray-200 p-4">
      ...
      <div class="mt-3 flex flex-wrap items-center gap-3">
        <PrimaryButton theme="success" @click="downloadTemplate">{{
          t.downloadTemplate
        }}</PrimaryButton>
        <label
          class="inline-flex cursor-pointer items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium"
        >
          {{ t.importCsv }}
          <input
            class="sr-only"
    ...
    <section class="flex flex-col gap-4">
      ...
      <article v-for="set in sets" :key="set.id" class="rounded-md border border-gray-200 p-4">
        <label class="grid gap-1 text-sm font-medium">
          {{ t.setName }}
          <input v-model="set.name" class="rounded-md border border-gray-300 p-2" type="text" />
        </label>
              ...
              <tr v-for="entry in set.entries" :key="entry.id" class="border-b border-gray-100">
                ...
                <td class="p-2">
                  <button
                    ...
                    type="button"
                    @click="deleteRow(set.id, entry.id)"
                  >
                    {{ t.deleteRow }}
                  </button>
                </td>
        ...
        <div class="mt-4 flex flex-wrap gap-3">
          ...
          <PrimaryButton theme="success" @click="downloadSet(set)">{{
            t.downloadCurrent
          }}</PrimaryButton>
          <PrimaryButton theme="danger" @click="deleteSetById(set.id)">{{
            t.deleteSet
          }}</PrimaryButton>
        </div>
      </article>
    ...
    <p
      ...
      :class="messageType === 'error' ? 'text-red-700' : 'text-emerald-700'"
    >
      {{ message }}
    </p>
  </main>
