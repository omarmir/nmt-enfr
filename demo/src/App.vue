    ...
    <header class="flex flex-col gap-4 border-b border-gray-200 pb-6">
      <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        ...
        <RouterLink
          ...
          :to="{ name: localizedRouteName, params: { locale: otherLocale } }"
        >
          {{ t.language }}
        </RouterLink>
      </div>
      <nav class="flex gap-4 text-sm font-medium">
        ...
        <RouterLink
          class="text-blue-700 hover:underline"
          :to="{ name: 'glossary', params: { locale } }"
        >
          {{ t.glossary }}
        </RouterLink>
        <RouterLink
          class="text-blue-700 hover:underline"
          :to="{ name: 'methodology', params: { locale } }"
        >
          {{ t.methodology }}
        </RouterLink>
      </nav>
...
const localizedRouteName = computed(() =>
  route.name === 'glossary' || route.name === 'methodology' ? route.name : 'home',
)
