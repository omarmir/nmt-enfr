...
let activeTranslationDtype = null;
const glossaryAnchorCache = new Map();

async function getTransformers() {
  if (transformersModule) return transformersModule;
  transformersModule = await import(config.transformersUrl);
  const { env } = transformersModule;

  if (config.modelsBaseUrl) {
    env.localModelPath = withTrailingSlash(config.modelsBaseUrl);
  }
  ...
  if (env.backends?.onnx?.wasm) {
    if (config.wasmBaseUrl) {
      env.backends.onnx.wasm.wasmPaths = withTrailingSlash(config.wasmBaseUrl);
    }
...
async function disposeTranslationModel() {
  ...
  activeTranslationDtype = null;
}

async function translateItem(item, model, dtype, generation) {
  if (!translator || activeTranslationModel !== resolveModelId(model) || activeTranslationDtype !== dtype) {
    await loadTranslationModel(model, dtype, false);
  }

  const originalInput = typeof item.text === "string" ? item.text.trim() : "";
  const sourceText = item.glossary?.text || item.text || "";
  ...
  if (originalInput && item.glossary?.replacements?.length) {
    const constrained = await translateWithGlossaryConstraints(originalInput, item.glossary.replacements, generation);
    if (constrained) return constrained;
  }

  const output = await translator(input, generationOptions(generation));
...
async function translateWithGlossaryConstraints(input, replacements, generation) {
  ...
  const startedAt = now();
  const baseline = await translator(input, generationOptions(generation));
  const baselineText = baseline?.[0]?.translation_text || "";
  if (!constraints.length) return cleanupGlossaryOutput(baselineText);
  if (containsAllGlossaryTargets(baselineText, constraints)) {
    logGlossaryConstraintResult({ strategy: "baseline", input, constraints, startedAt, candidates: [{ strategy: "baseline", text: baselineText }] });
  ...
  const beamStrategies = [
    ["beam-soft-bias-6", createBeamAwareGlossaryProcessor(constraints, { bias: 6, forceAfterTokens: 0 }), beamWidth],
    ["beam-soft-bias-9", createBeamAwareGlossaryProcessor(constraints, { bias: 9, forceAfterTokens: 0 }), beamWidth],
  ];
  ...
  const beamSelected = selectGlossaryCandidate(baselineText, candidates, constraints, { allowPartial: true });
  if (beamSelected) {
...
function generationOptions(generation = {}, overrides = {}) {
  return {
...
async function glossaryTranslationAnchors(constraint, generation) {
  ...
  if (!source) return [];
  const cacheKey = `${activeTranslationModel}:${activeTranslationDtype}:${source}`;
  if (glossaryAnchorCache.has(cacheKey)) return glossaryAnchorCache.get(cacheKey);

  const variants = [source, `the ${source}`];
  const anchors = new Set();
  for (const variant of variants) {
    const output = await translator(variant, generationOptions(generation, {
...
function anchorVariants(text) {
  ...
  const withoutArticle = cleaned.replace(/^(l['’]\s*|le\s+|la\s+|les\s+|un\s+|une\s+|des\s+|du\s+|de la\s+|the\s+|a\s+|an\s+)/i, "").trim();
...
function replaceFirstGlossaryAnchor(text, anchors, target) {
  ...
  return `${before}${adaptGlossaryTargetForContext(before, target)}${after}`;
}
...
function cleanupGlossaryOutput(text) {
  return capitalizeFirstLetter(String(text || "")
    ...
    .replace(/\b([Ss])\s+a\s+applique\b/g, "$1'applique")
    .replace(/\b[Aa]vant approbation\b/g, (match) => match[0] === "A" ? "Avant l'approbation" : "avant l'approbation")
    .replace(/\bque approbation\b/g, "que l'approbation")
...
function constraintStartBonus(content, constraint, constraints) {
  ...
  const orderPenalty = Math.max(0, orderIndex - satisfiedBefore) * 1.2;
  return -orderPenalty;
}
...
function createBeamAwareGlossaryProcessor(constraints, options = {}) {
  ...
  return (inputIds, logits) => {
    forEachLogitRow(inputIds, logits, ({ generated, row }) => {
      ...
      const shouldForceStart = forceAfterTokens > 0 && content.length >= forceAfterTokens;
      if (shouldForceStart) {
...
function createAppendBeforeEosProcessor(constraints) {
  return (inputIds, logits) => {
    forEachLogitRow(inputIds, logits, ({ generated, row }) => {
      ...
      const missing = missingGlossaryConstraints(content, constraints);
      if (!missing.length) return;
      const next = missing[0].tokenIds[0];
      if (shouldAppendGlossaryTerm(content, logits, row)) {
        forceLogitRow(logits, row, next);
      } else {
        suppressEos(logits, row);
      }
    });
...
function glossaryStrategyPenalty(strategy) {
  ...
  if (strategy === "beam-late-force") return 0.35;
  return 0;
}
...
function scoreGlossaryCandidate(baselineText, candidateText, constraints) {
  ...
  const partialAccepted = !reasons.length && glossaryCoverage > 0 && glossaryCoverage >= Math.min(0.68, 1 / Math.max(1, constraints.length));
  const appendedTailPenalty = glossaryTailPenalty(candidate, constraints);
  const packingPenalty = glossaryPackingPenalty(candidate, constraints);
  const articlePenalty = glossaryArticlePenalty(candidate);
  ...
  const score = 10 + exactBonus - missingPenalty - (lengthPenalty * 2.5) - (anchorPenalty * 1.5) - (appendedTailPenalty * 2.0) - (packingPenalty * 1.4) - (articlePenalty * 1.8) - punctuationPenalty;
...
function glossaryTargetVariants(target) {
  ...
  if (normalized === "conge annuel") variants.add("conges annuels").add("conges annuel");
  ...
  if (normalized === "compte utilisateur") {
    ...
    variants.add("comptes d utilisateur");
    variants.add("comptes d utilisateurs");
  }
  if (normalized === "liste de controle d acces") {
    variants.add("liste des controles d acces");
    variants.add("liste de controles d acces");
    variants.add("liste des controles d access");
    variants.add("liste de controle d access");
  }
...
function glossaryTailPenalty(candidateText, constraints) {
  const normalized = normalizeGlossaryText(candidateText);
  const sentenceParts = String(candidateText || "").split(/[.!?]+/).map(normalizeGlossaryText).filter(Boolean);
  const finalPart = sentenceParts.at(-1) || normalized;
  let penalty = 0;
  for (const constraint of constraints) {
    const target = normalizeGlossaryText(constraint.target);
    if (sentenceParts.length > 1 && (finalPart === target || finalPart.endsWith(` ${target}`))) penalty += 1.35;
  }
  return penalty;
}

function glossaryPackingPenalty(candidateText, constraints) {
  const normalized = normalizeGlossaryText(candidateText);
  const spans = constraints
    .map((constraint) => {
      const target = normalizeGlossaryText(constraint.target);
      const start = normalized.indexOf(target);
      return start === -1 ? null : { start, end: start + target.length };
    })
    .filter(Boolean)
    .sort((a, b) => a.start - b.start);
  let penalty = 0;
  for (let index = 1; index < spans.length; index++) {
    const gap = normalized.slice(spans[index - 1].end, spans[index].start).replace(/[^\p{L}\p{N}]+/gu, "");
    if (!gap) penalty += 0.9;
  }
  return penalty;
}

function glossaryArticlePenalty(candidateText) {
  const normalized = normalizeGlossaryText(candidateText);
  let penalty = 0;
  if (/\bla journal\b/.test(normalized)) penalty += 0.8;
  if (/\bune journal\b/.test(normalized)) penalty += 0.8;
  if (/\bdes compte\b/.test(normalized)) penalty += 0.5;
  return penalty;
}
...
function logGlossaryConstraintResult({ strategy, input, constraints, startedAt, candidates }) {
  console.info(JSON.stringify({
    type: "nmt-enfr-glossary-constraints",
    strategy,
...
async function translateBatch(message) {
  ...
  const dtype = message.dtype || "q8";
  const generation = message.generation || {};
  for (const item of items) {
    const result = await translateItem(item, model, dtype, generation);
    self.postMessage({
      status: "result",
...
function resolveModelId(model) {
  const name = String(model || "opus-mt-en-fr");
  if (config.modelsBaseUrl || name.includes("/")) return name;
  return `Xenova/${name}`;
}
...
self.addEventListener("message", async (event) => {
  ...
  try {
    if (message.task === "configure") {
      config = { ...config, ...(message.config || {}) };
      return;
    }

    if (message.task === "model") {
      await loadTranslationModel(message.model || "opus-mt-en-fr", message.dtype || "q8", true);
      return;
    }
